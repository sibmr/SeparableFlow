...
def create_kitti_submission(model, iters=24, output_path='kitti_submission'):
    ...
    test_dataset = datasets.KITTI(split='testing', root=file_path,  aug_params=None)
...
@torch.no_grad()
def create_kitti2012_submission(model, iters=24, output_path='kitti2012_submission'):
    """ Create submission for the Sintel leaderboard """
    model.eval()
    file_path = "/export/work/feihu/kitti2012"
    test_dataset = datasets.KITTI2012(split='testing', root=file_path,  aug_params=None)
...
@torch.no_grad()
def validate_kitti2012(model, iters=24):
    """ Peform validation using the KITTI-2015 (train) split """
    model.eval()
    file_path = "/export/work/feihu/kitti2012"
    val_dataset = datasets.KITTI2012(split='training', root=file_path)
